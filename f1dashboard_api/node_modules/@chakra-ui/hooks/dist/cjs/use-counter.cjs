...
function useCounter(props = {}) {
  ...
  const clamp = react.useCallback(
    (value2) => {
      ...
      if (keepWithinRange) {
        nextValue = number.clampValue(nextValue, min, max);
      }
      return number.toPrecision(nextValue, precision);
    },
...
function getDecimalPlaces(value, step) {
  return Math.max(number.countDecimalPlaces(step), number.countDecimalPlaces(value));
}
function cast(value, step, precision) {
  ...
  return number.toPrecision(parsedValue, precision ?? decimalPlaces);
}
