...
var focusable = require('@chakra-ui/utils/focusable');
var owner = require('@chakra-ui/utils/owner');
var tabbable = require('@chakra-ui/utils/tabbable');
...
function preventReturnFocus(containerRef) {
  ...
  if (!el)
    return false;
  const activeElement = owner.getActiveElement(el);
  if (!activeElement)
    return false;
  if (el.contains(activeElement))
    return false;
  if (tabbable.isTabbable(activeElement))
    return true;
  return false;
}
...
function useFocusOnShow(target, options = defaultOptions) {
  ...
  const onFocus = react.useCallback(() => {
    ...
    if (focusRef?.current) {
      ...
      const tabbableEls = focusable.getAllFocusable(element);
