...
var addPointerEvent = require('@chakra-ui/utils/add-pointer-event');
var eventPoint = require('@chakra-ui/utils/event-point');
var isEvent = require('@chakra-ui/utils/is-event');
...
class PanEvent {
  constructor(event, handlers, threshold) {
    ...
    if (isEvent.isMultiTouchEvent(event))
      return;
    ...
    const info = { point: eventPoint.getEventPoint(event) };
    const { timestamp } = sync.getFrameData();
    this.history = [{ ...info.point, timestamp }];
    const { onSessionStart } = handlers;
    onSessionStart?.(event, getPanInfo(info, this.history));
    this.removeListeners = pipe(
      addPointerEvent.addPointerEvent(this.win, "pointermove", this.onPointerMove),
      addPointerEvent.addPointerEvent(this.win, "pointerup", this.onPointerUp),
      addPointerEvent.addPointerEvent(this.win, "pointercancel", this.onPointerUp)
    );
  }
