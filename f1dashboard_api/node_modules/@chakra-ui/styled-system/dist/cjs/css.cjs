...
var merge = require('@chakra-ui/utils/merge');
...
function getCss(options) {
  ...
  const css2 = (stylesOrFn, nested = false) => {
    const _styles = runIfFn.runIfFn(stylesOrFn, theme);
    const styles = expandResponsive.expandResponsive(_styles)(theme);
    let computedStyles = {};
    for (let key in styles) {
      const valueOrFn = styles[key];
      let value = runIfFn.runIfFn(valueOrFn, theme);
      if (key in pseudos) {
        key = pseudos[key];
      }
      ...
      if (is.isObject(value)) {
        ...
        computedStyles[key] = merge.mergeWith(
          {},
          computedStyles[key],
          css2(value, true)
        );
      ...
      const configProperty = runIfFn.runIfFn(config?.property, theme);
      if (!nested && config?.static) {
        const staticStyles = runIfFn.runIfFn(config.static, theme);
        computedStyles = merge.mergeWith({}, computedStyles, staticStyles);
      }
      ...
      if (configProperty) {
        if (configProperty === "&" && is.isObject(rawValue)) {
          computedStyles = merge.mergeWith({}, computedStyles, rawValue);
        } else {
          computedStyles[configProperty] = rawValue;
        }
        continue;
      }
      if (is.isObject(rawValue)) {
        computedStyles = merge.mergeWith({}, computedStyles, rawValue);
        continue;
      }
