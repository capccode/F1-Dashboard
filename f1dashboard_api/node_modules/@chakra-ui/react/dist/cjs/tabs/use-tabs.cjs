...
var children = require('@chakra-ui/utils/children');
var context = require('@chakra-ui/utils/context');
var lazy = require('@chakra-ui/utils/lazy');
...
const [TabsProvider, useTabsContext] = context.createContext({
  name: "TabsContext",
...
function useTabList(props) {
  ...
  return {
    ...
    onKeyDown: callAll.callAllHandlers(props.onKeyDown, onKeyDown)
  };
}
function useTab(props) {
  ...
  const clickableProps = useClickable.useClickable({
    ...htmlProps,
    ref: useMergeRefs.mergeRefs(register, props.ref),
    isDisabled,
    isFocusable,
    onClick: callAll.callAllHandlers(props.onClick, onClick)
  });
  const type = "button";
  return {
    ...clickableProps,
    ...
    onFocus: isDisabled ? void 0 : callAll.callAllHandlers(props.onFocus, onFocus)
  };
}
const [TabPanelProvider, useTabPanelContext] = context.createContext({});
function useTabPanels(props) {
  const context = useTabsContext();
  const { id, selectedIndex } = context;
  const validChildren = children.getValidChildren(props.children);
  const children$1 = validChildren.map(
  ...
  return { ...props, children: children$1 };
}
function useTabPanel(props) {
  ...
  const shouldRenderChildren = lazy.lazyDisclosure({
