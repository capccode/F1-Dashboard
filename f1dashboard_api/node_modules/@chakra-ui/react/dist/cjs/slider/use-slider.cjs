...
var useLatestRef = require('@chakra-ui/hooks/use-latest-ref');
var useMergeRefs = require('@chakra-ui/hooks/use-merge-refs');
var usePanEvent = require('@chakra-ui/hooks/use-pan-event');
var useSize = require('@chakra-ui/hooks/use-size');
...
var number = require('@chakra-ui/utils/number');
...
function useSlider(props) {
  ...
  const onChangeStart = useCallbackRef.useCallbackRef(onChangeStartProp);
  const onChangeEnd = useCallbackRef.useCallbackRef(onChangeEndProp);
  const getAriaValueText = useCallbackRef.useCallbackRef(getAriaValueTextProp);
  const isReversed = sliderUtils.getIsReversed({
    isReversed: isReversedProp,
    direction,
    orientation
  });
  const [computedValue, setValue] = useControllableState.useControllableState({
    value: valueProp,
  ...
  const value = number.clampValue(computedValue, min, max);
  const reversedValue = max - value + min;
  const trackValue = isReversed ? reversedValue : value;
  const thumbPercent = number.valueToPercent(trackValue, min, max);
  const isVertical = orientation === "vertical";
  const stateRef = useLatestRef.useLatestRef({
    min,
    max,
    step,
    isDisabled,
  ...
  const getValueFromPointer = react.useCallback(
    (event) => {
      ...
      let nextValue = number.percentToValue(percent, state2.min, state2.max);
      if (state2.step) {
        nextValue = parseFloat(
          number.roundValueToStep(nextValue, state2.min, state2.step)
        );
      }
      nextValue = number.clampValue(nextValue, state2.min, state2.max);
      return nextValue;
    },
  ...
  const constrain = react.useCallback(
    (value2) => {
      ...
      if (!state2.isInteractive)
        return;
      value2 = parseFloat(number.roundValueToStep(value2, state2.min, oneStep));
      value2 = number.clampValue(value2, state2.min, state2.max);
      setValue(value2);
    },
  ...
  const thumbSize = useSize.useSize(thumbRef);
  ...
  usePanEvent.usePanEvent(rootRef, {
  ...
  const getRootProps = react.useCallback(
    (props2 = {}, ref = null) => {
      return {
        ...props2,
        ...htmlProps,
        ref: useMergeRefs.mergeRefs(ref, rootRef),
        tabIndex: -1,
        "aria-disabled": attr.ariaAttr(isDisabled),
        "data-focused": attr.dataAttr(isFocused),
        style: {
          ...props2.style,
          ...rootStyle
        }
  ...
  const getTrackProps = react.useCallback(
    (props2 = {}, ref = null) => {
      return {
        ...props2,
        ref: useMergeRefs.mergeRefs(ref, trackRef),
        id: trackId,
  ...
  const getThumbProps = react.useCallback(
    (props2 = {}, ref = null) => {
      return {
        ...props2,
        ref: useMergeRefs.mergeRefs(ref, thumbRef),
        role: "slider",
        tabIndex: isInteractive ? 0 : void 0,
        id: thumbId,
        "data-active": attr.dataAttr(isDragging),
        ...
        onFocus: callAll.callAllHandlers(props2.onFocus, () => setFocused(true)),
        onBlur: callAll.callAllHandlers(props2.onBlur, () => setFocused(false))
      };
    },
    [
      isInteractive,
  ...
  const getMarkerProps = react.useCallback(
    (props2, ref = null) => {
      ...
      const markerPercent = number.valueToPercent(props2.value, min, max);
      const markerStyle = {
        position: "absolute",
        pointerEvents: "none",
        ...orient({
      ...
      return {
        ...
        "aria-hidden": true,
        "data-disabled": attr.dataAttr(isDisabled),
        "data-invalid": attr.dataAttr(!isInRange),
        "data-highlighted": attr.dataAttr(isHighlighted),
        style: {
          ...props2.style,
          ...markerStyle
        }
