...
function useRangeSlider(props) {
  ...
  const [valueState, setValue] = useControllableState.useControllableState({
    value: valueProp,
  ...
  const value = valueState.map((val) => number.clampValue(val, min, max));
  ...
  const thumbPercents = thumbValues.map((val) => number.valueToPercent(val, min, max));
  const isVertical = orientation === "vertical";
  const trackRef = react.useRef(null);
  const rootRef = react.useRef(null);
  const thumbRects = useSize.useSizes({
    getNodes() {
  ...
  const getValueFromPointer = react.useCallback(
    (event) => {
      ...
      if (isReversed)
        percent = 1 - percent;
      return number.percentToValue(percent, min, max);
    },
  ...
  const actions = react.useMemo(
    () => ({
      setValueAtIndex(index, val) {
        ...
        val = parseFloat(number.roundValueToStep(val, bounds.min, oneStep));
        val = number.clampValue(val, bounds.min, bounds.max);
  ...
  const getThumbProps = react.useCallback(
    (props2, ref = null) => {
      ...
      return {
        ...
        "data-active": attr.dataAttr(isDragging && activeIndex === index),
        ...
        onFocus: callAll.callAllHandlers(props2.onFocus, () => {
          setFocused(true);
          setActiveIndex(index);
        }),
        onBlur: callAll.callAllHandlers(props2.onBlur, () => {
          setFocused(false);
          setActiveIndex(-1);
        })
      };
  ...
  const getMarkerProps = react.useCallback(
    (props2, ref = null) => {
      ...
      let percent = number.valueToPercent(v, min, max);
